...
class TestCLI(unittest.TestCase):
    ...
    def test_validate_config_valid(self):
        ...
        # Create valid config
        config = {
            'random': {'n': 5, 'length': 100},
            'simple': {'n': 3, 'repeat': 'CAG', 'units': 20}
        }
        ...
        try:
            ...
            self.assertEqual(result.exit_code, 0)
            # Click captures logging output, not stdout, so we check exit code instead
        finally:
    ...
    def test_validate_config_invalid(self):
        ...
        config = {
            'random': {'n': -1, 'length': 0},  # Invalid values
    ...
    def test_simulate_command_success(self, mock_registry_class, mock_generator_class, 
        ...
        mock_selector.select_reads.return_value = [
            (Mock(query_name='read1'), Mock()),
            (Mock(query_name='read2'), Mock())
        ]
        
        mock_inserter = Mock()
        mock_inserter_class.return_value = mock_inserter
        mock_inserter.insert_random_mode.return_value = (
            [Mock(), Mock()],  # insertion_records
            [Mock(), Mock()],  # modified_sequences
            {'successful_insertions': 2, 'total_attempted': 2}  # skip_stats
        )
